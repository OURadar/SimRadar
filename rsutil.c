...
static void RS_test_by_flags(const uint32_t flag, const uint32_t verb) {
    float density = 0.0f;
    unsigned int N = 0;
...
void RS_test_by_number(const int number, const void *arg) {
    ...
    switch (number) {
        case 0:
            verb = *(int *)arg;
            RS_test_by_flags(TEST_SIMPLE, verb);
            break;
        case 1:
            verb = *(int *)arg;
            RS_test_by_flags(TEST_KERNEL_MASK, verb);
            break;
        case 2:
            verb = *(int *)arg;
            RS_test_by_flags(TEST_ALL, verb);
            break;
        case 4:
            ...
            break;
        case 11:
            RS_test_by_flags(TEST_IO, verb);
            break;
        case 12:
            RS_test_by_flags(TEST_ADVANCE_TIME_GPU, verb);
            break;
        case 13:
            RS_test_by_flags(TEST_MAKE_PULSE_GPU, verb);
            break;
        case 14:
            RS_test_by_flags(TEST_MAKE_PULSE_GPU_PASS_1, verb);
            break;
        case 15:
            RS_test_by_flags(TEST_MAKE_PULSE_GPU_PASS_2, verb);
            break;
        case 16:
            RS_test_by_flags(TEST_EL_ATTS, verb);
            break;
        case 17:
            RS_test_by_flags(TEST_DB_ATTS, verb);
            break;
        case 18:
            RS_test_by_flags(TEST_SIG_AUX, verb);
            break;
        default:
...
char *RS_test_by_number_description(const int indent) {
    ...
    char helpText[] =
    " 0 - Simple framework test using GPU(s)\n"
    " 1 - Advanced framework tests using GPU(s)\n"
    " 2 - Simple framework test using CPU\n"
    " 3 - Advanced framework tests using both GPUs and CPUs\n"
    "\n"
    ...
    while ((opt = getopt_long(argc, (char * const *)argv, str, long_options, &long_index)) != -1) {
        switch (opt) {
            ...
            case 't':
                ...
                k = atoi(optarg);
                RS_test_by_number(k, argc == optind ? (char *)&verbose : argv[optind]);
                exit(EXIT_SUCCESS);
                break;
            case 'v':
                verbose++;
