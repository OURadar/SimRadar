...
enum {
	...
	TEST_GPU_ALL                = TEST_ADVANCE_TIME_GPU | TEST_MAKE_PULSE_GPU_PASS_1 | TEST_MAKE_PULSE_GPU_PASS_2 | TEST_DOWNLOAD,
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "ac12gvn:s:h?")) != -1) {
		switch (c) {
			case 'a':
				test = TEST_ALL;
				break;
...
#define FMT2  "%30s  %8.4f ms  (%.2f GB/s)\n"
	...
	if (test & TEST_MAKE_PULSE_GPU) {
		...
		for (i=0; i<N; i++)
			RS_make_pulse(H);
		gettimeofday(&t2, NULL);
		printf(FMT, "RS_make_pulse()", 1.0e3f * DTIME(t1, t2) / (float)N);
	}
